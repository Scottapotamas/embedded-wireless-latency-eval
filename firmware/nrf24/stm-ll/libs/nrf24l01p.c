...
static uint8_t read_register(uint8_t reg)
{
    ...
    while (!LL_SPI_IsActiveFlag_RXNE(SPI1));
    read_val = LL_SPI_ReceiveData8(SPI1);
...
//    while (!LL_SPI_IsActiveFlag_TXE(SPI1));
    ...
    while (LL_SPI_IsActiveFlag_BSY(SPI1));
    while (!LL_SPI_IsActiveFlag_TXE(SPI1));
    LL_SPI_TransmitData8(SPI1, write_val);
    while(!LL_SPI_IsActiveFlag_TXE(SPI1));
    while (LL_SPI_IsActiveFlag_BSY(SPI1));
...
uint8_t nrf24l01p_read_rx_fifo(uint8_t* rx_payload)
{
    ...
    uint8_t status;
    uint8_t bytes = NRF24L01P_PAYLOAD_LENGTH;
    ...
    while(bytes--)
    {
        while (LL_SPI_IsActiveFlag_BSY(SPI1));
        while (!LL_SPI_IsActiveFlag_TXE(SPI1));
        LL_SPI_TransmitData8(SPI1, NRF24L01P_CMD_NOP);
        while (!LL_SPI_IsActiveFlag_RXNE(SPI1));
        *rx_payload++ = LL_SPI_ReceiveData8(SPI1);
    }
...
uint8_t nrf24l01p_write_tx_fifo(uint8_t* tx_payload)
{
    ...
    while(bytes--)
    {
            while (LL_SPI_IsActiveFlag_BSY(SPI1));
            while (!LL_SPI_IsActiveFlag_TXE(SPI1));
            LL_SPI_TransmitData8(SPI1, *tx_payload++);
            while (!LL_SPI_IsActiveFlag_RXNE(SPI1));
