...
#define SERVER 0
#define CLIENT 1
// #define WS_MODE (SERVER)
#define WS_MODE (CLIENT)
...
#if WS_MODE == SERVER
    websocket_server_register_user_evt_queue(bench_evt_queue);
    xTaskCreate(websocket_server_task, "ws_server", 4096, NULL, 5, NULL);
#else
    websocket_client_register_user_evt_queue(bench_evt_queue);
    xTaskCreate(websocket_client_task, "ws_client", 4096, NULL, 5, NULL);
#endif
...
#if WS_MODE == SERVER
            websocket_server_send_payload( &test_payload[bytes_sent], bytes_to_send );
            bytes_pending = bytes_to_send;
#else
            websocket_client_send_payload( &test_payload[bytes_sent], bytes_to_send );
#endif
